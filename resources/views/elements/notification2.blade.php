<!-- BEGIN NOTIFICATION --->
    <div class="pb-5 pb-md-0">
        <div class="bg-primary mb-md-n5 mb-5 p-md-5 p-4 mt-n5 mt-md-5 text-white justify-content-center">
            <div class="row align-items-center">
                <div class="col-md-8 col-12">
                    <h3>¿Tenés dudas con respecto a los planes?</h3>
                    <p>En nuestra zona de preguntas frecuentes podés quitarte todas tus dudas.</p>
                </div>
                <div class="col-md-3 col-12 d-flex justify-content-start">
                    <a href="{{ route('plans') }}" class="btn btn-outline-light p-3 px-4 rounded-pill d-flex justify-content-start focom-notification-bottom-width">
                        <i class="fas fa-mouse-pointer fa-lg pr-2 pt-1"></i>
                        <span>
                            <b>Ir ahora</b>
                        </span>
                    </a>
                </div>
            </div>
        </div>
    </div>
<!-- END NOTIFICATION --->