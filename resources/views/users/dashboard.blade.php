...
<div class="py-4 d-flex justify-content-between">
    ...
    <div class="my-auto">
        <a class="btn btn-outline-primary" href="{{ route('articles.create') }}">Publicar<i class="fas fa-plus fa-sm pl-2"></i></a>
        <a class="btn btn-secondary" href="{{route('user.config')}}">Configuraciones<i class="fas fa-cog fa-sm pl-2"></i></a>
    </div>
</div>
<!-- BEGIN Si es particular no mostrar -->
<span class="lead pl-2">{{ auth()->user()->role->name }} - Plan {{ auth()->user()->plan->name }}</span> 
<!-- END Si es particular no mostrar -->
<div class="border rounded px-4 py-4 pt-0 d-flex justify-content-between">
    <span class="pt-1"><span class="h5">{{ auth()->user()->articles()->count() }}</span><a href=""></a> Publicaciones activas</span>
